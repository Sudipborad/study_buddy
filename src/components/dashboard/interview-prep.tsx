...
import { generateInterviewQuiz } from '@/ai/flows/generate-interview-quiz';
...
export function InterviewPrep() {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      const result = await generateInterviewQuiz({ skills: cvData.skills.filter(s => s) });
      if (result.questions && result.questions.length > 0) {
        setQuestions(result.questions);
      } else {
        toast({ variant: 'destructive', title: 'Error', description: 'Could not generate an interview quiz from the provided skills.' });
      }
    } catch (error) {
      console.error(error);
  ...
  if (!cvData?.skills || cvData.skills.length === 0 || cvData.skills.every(s => !s)) {
      return (
           <Card className="w-full max-w-lg mx-auto mt-10 text-center">
              <CardHeader>
                  <CardTitle className="font-headline">No Skills Found</CardTitle>
                  <CardDescription>
                      Please add some skills to your resume first to generate interview questions.
                  </CardDescription>
              </CardHeader>
              <CardContent>
                  <Button asChild>
  ...
  if (isLoading) {
    return (
      <div className="text-center p-10">
        ...
        <p className="mt-4 text-muted-foreground">The AI is generating your interview questions...</p>
      </div>
    );
  }
  ...
  if (questions.length === 0 && !isLoading) {
     return (
        <Card className="text-center p-10 bg-secondary/50 border-dashed">
            ...
            <p className="text-muted-foreground mt-2">The AI was unable to create a quiz from the skills in your resume. Try adding more specific skills.</p>
  ...
  if (quizFinished) {
    return (
       <Card className="w-full max-w-2xl mx-auto text-center">
        <CardHeader>
          <CardTitle className="font-headline text-3xl">Interview Prep Complete!</CardTitle>
